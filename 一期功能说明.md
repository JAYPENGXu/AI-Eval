...
短期记忆和会话摘要只服务于当前会话创建者，不是组织共享数据。系统采用两层隔离：

1. 后端 `ChatSessionViewSet.get_queryset()` 同时限制 `owner=request.user` 和当前用户可访问的 KnowledgeBase；详情、消息、SSE 和删除都通过同一个授权父对象解析，其他成员猜测旧会话 ID 只能得到 `404`。
2. 前端身份切换是原子过程：先显示身份加载门禁并取消旧请求，再清空聊天、Citation、反馈草稿、Trace、评测、Agent、文档和知识库状态，最后加载新账号工作区。

这里不能只依赖“退出后隐藏页面”。`App.vue` 本身不会随 `AuthManager` 的 slot 切换销毁，如果不主动清理，上一账号的内存状态可能在新账号 bootstrap 期间短暂显示；当新账号会话列表为空时还可能持续残留。
...
读取本地保存的 session_id
-> 重新请求当前身份的授权会话列表
-> session_id 存在于列表才加载消息
-> 不存在或列表为空则清空 session/messages
```

浏览器不保存消息正文、引用内容或会话摘要。后端即使允许用户访问同一知识库，也不会因此允许查看其他成员的聊天记录。
...
这个阈值由 `SESSION_SUMMARY_TRIGGER_MESSAGES` 配置。达到条件后，后端会提交 Celery 摘要任务调用便宜模型，不阻塞用户看到流式回答。
...
前端可以直接运行评测。后端创建 queued `RagEvalRun` 并提交 Celery，前端轮询持久化状态。
...
评测 Run 由 Celery 执行，保存 task ID、心跳、重试和失败信息；Worker 重启时通过任务确认与 stale reconciliation 恢复。
...
- `test_access_control.py`：同组织不同成员的会话列表隔离，以及旧 Session ID 的详情、消息和删除接口均返回 404。
- `frontend/e2e/demo-personas.spec.ts`：浏览器真实执行研发 Persona 退出并切换供应商，验证旧会话不会残留在当前会话或历史列表中。
